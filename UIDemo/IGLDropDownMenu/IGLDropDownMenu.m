...
- (void)expandView
{
    ...
    if ([_delegate respondsToSelector:@selector(willExpandDropDownMenu:)]) {
        [_delegate willExpandDropDownMenu:self];
    }
    ...
    for (int i = (int)self.dropDownItems.count - 1; i >= 0; i--) {
        ...
        if (self.shouldUseSpringAnimation && IOS7_OR_GREATER) {
            ...
            [UIView animateWithDuration:self.animationDuration * 2 delay:delay usingSpringWithDamping:0.5 initialSpringVelocity:2.0 options:self.animationOption animations:^{
                ...
                if (i == 0) {
                    [self updateSelfFrame];
                    if ([_delegate respondsToSelector:@selector(didExpandDropDownMenu:)]) {
                        [_delegate didExpandDropDownMenu:self];
                    }
                }
            }];
#endif
...
{
    // fold the view
    if ([_delegate respondsToSelector:@selector(willCollapseDropDownMenu:)]) {
        [_delegate willCollapseDropDownMenu:self];
    }
    ...
    for (int i = (int)self.dropDownItems.count - 1; i >= 0; i--) {
        ...
        if (self.shouldUseSpringAnimation && IOS7_OR_GREATER) {
            ...
            [UIView animateWithDuration:self.animationDuration delay:delay usingSpringWithDamping:1.0 initialSpringVelocity:2.0 options:self.animationOption animations:^{
                ...
                if (i == 0) {
                    [self updateSelfFrame];
                    if ([_delegate respondsToSelector:@selector(didCollapseDropDownMenu:)]) {
                        [_delegate didCollapseDropDownMenu:self];
                    }
                }
            }];
#endif
